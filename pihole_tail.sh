...
do
  ...
  ## grab last line of forwarded only log, then grab relevant fields
  line_forward2=$(cat /var/log/pihole.log | grep "forwarded" | tail -n 1)
  line_forward3=$(echo $line_query2 | awk '{print $5 , $6 , $8}')
  ...
  ## - specifically needed as queries appear too quickly in log
  ## - test for 'wpad.LandOfOz' is specific to my network; can remove
  if [ "$line_forward3" !=  "$line_forward1" -a "$(echo $line_forward3 | awk '{print $2}')" != "wpad.LandOfOz" ]
    then
      ## atomise line fields
      ## field 1: type
      ## field 2: human readable address
      ## field 3: ip address
      #f_first=$(echo $line_forward3 | awk '{print $1}')
      f_second=$(echo $line_forward3 | awk '{print $2}')

      ## determine field 2 length
      f_second_len=${#f_second}

      ## start new line
      printf "%b\n"

      ## colourised print
      if [ $(echo "$f_second_len >= 34" | bc) -eq 1 ]
        then
          f_second=$(echo $f_second | cut -c -31)
          printf "%6s" "${CYAN}${BRIGHT}forwd${NORMAL} " "$f_second..."
        else
          printf "%6s" "${CYAN}${BRIGHT}forwd${NORMAL} " "$f_second"
      fi
      ## update last line
      line_forward1=$line_forward3
  fi

  ## compare latest line proper with previous
  ## - test for 'wpad.LandOfOz' is specific to my network; can remove
  if [ "$line3" !=  "$line1" -a "$(echo $line3 | awk '{print $2}')" != "wpad.LandOfOz" ]
    then
      ## atomise line fields
      ## field 1: type
      ## field 2: human readable address
        ...
        then
          if [ $(echo "$second_len >= 34" | bc) -eq 1 ]
            then
              ...
              printf "%s" "${GREEN}${BRIGHT}reply{NORMAL} $second..."
            else
              printf "%s" "${GREEN}${BRIGHT}reply${NORMAL} $second"
          fi
      elif [ "$first" = "cached" ]
        then
          if [ $(echo "$second_len >= 33" | bc) -eq 1 ]
            then
              ...
              printf "%s" "${YELLOW}${BRIGHT}cache${NORMAL} $second..."
            else
              printf "%s" "${YELLOW}${BRIGHT}cache${NORMAL} $second"
          fi
        ...
        then
          if [ $(echo "$second_len >= 32" | bc) -eq 1 ]
            then
              ...
              printf "%s" "${RED}${BRIGHT}block${NORMAL} $second..."
            else
              printf "%s" "${RED}${BRIGHT}block${NORMAL} $second"
          fi
      else
